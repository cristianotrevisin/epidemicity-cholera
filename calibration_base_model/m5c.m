function [ModPred,times_out, y] = m4c(x,Extra)
    ...
    y0(5,initial_points) = p.sigma*I_initial; %reported cases @ beginning
    y0(4,initial_points) = y0(2,initial_points)*p.theta/p.muB./POPnodes(initial_points)';

    %run model
    y=SIB(p,nnodes,POPnodes,fluxes,rainfall_day,cati,ocv,time_model,y0);
    cumcases_AD1_t=y(:,5:11:end)';
    ...
    function ytop=SIB(p,nnodes,H,fluxes,rainfall_day,cati,ocv,tspan,y0)
        ...
        function dy=eqs(t,y)
            ...
            Yeff1 = ((cati(index_t,:) - cati(index_t - keff1 +1,:))'./H/p.a1).^p.b1;
            ...
            Yeff2 = ((cati(index_t,:) - cati(index_t - keff2 +1,:))'./H/p.a2).^p.b2;
            ...
            FI=((1-p.m)*y(6:14:end)./(1+y(6:14:end))+temp*p.m).*(beta_t);
            ...
            dy(2:14:end)=p.sigma.*FI.*y(1:11:end) - (p.gamma+p.alpha+p.mu)*y(2:14:end);       %I
            dy(3:14:end)=(1-p.sigma).*FI.*y(1:14:end) - (p.gamma+p.mu+rv1)*y(3:14:end);       %A
            dy(4:14:end)=p.gamma*(y(2:6:end)+y(3:6:end)) - (p.mu+p.rho+rv1).*y(4:14:end);       %R
            dy(5:14:end)=-(p.muB).*y(5:6:end)+theta_t.*GI./H;  %B
            dy(6:14:end)=p.sigma.*FI.*(y(1:14:end)+(1-ocv.eta_1d(:,index_t)).*y(7:14:end)+(1-ocv.eta_2d(:,index_t)).*y(11:14:end));   %Cum(I)  
            
            
            
            
%             dy(1:11:end)=p.mu*(H-y(1:11:end))-(FI+rv1).*y(1:11:end)+p.rho*y(3:11:end);        %S
%             dy(2:11:end)=p.sigma.*FI.*y(1:11:end)-(p.gamma+p.alpha+p.mu)*y(2:11:end);       %I
%             dy(3:11:end)=(1-p.sigma).*FI.*y(1:11:end)+ p.gamma*y(2:11:end) - (p.mu+p.rho+rv1).*y(3:11:end);       %R
%             dy(4:11:end)=-(p.muB).*y(4:11:end)+theta_t.*(y(2:11:end)+y(7:11:end)+y(10:11:end))./H;  %B
%             dy(5:11:end)=p.sigma.*FI.*(y(1:11:end)+(1-ocv.eta_1d(:,index_t)).*y(6:11:end)+(1-ocv.eta_2d(:,index_t)).*y(9:11:end));   %Cum(I)  
%             
