function [Rt, et, all_r, all_e] = diagnosis(y,vec1,vec2)
    Rt = zeros(1,size(y,1));
    et = zeros(1,size(y,1));
    all_r = zeros(30,size(y,1));
    all_e = zeros(30,size(y,1));
    ...
    ocv.rv_1d=ocv.rv_1d(index_ocv,:)';
    ocv.rv_2d=ocv.rv_2d(index_ocv,:)';
    ...
    for i = 1:size(y,1)
        ...
        all_r(:,i) = sort(real(eig(full(NGM))),'descend');
        % EPIDEMICITY
        
        et(i) = eigs(HR,1,'largestreal');
        all_e(:,i) = sort(eig(HR), 'descend');
    end
end