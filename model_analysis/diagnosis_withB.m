...
elseif (nargin ~= 6) && (scenario_npi == 5 || scenario_npi == 6 ||scenario_npi == 7)
    ...
    for i = 1:size(y,1)
        ...
        TV1 = ((1-p.m)*U + p.m*fluxes)*s1*betaM*DD*(U-eta1);
        TV2 = ((1-p.m)*U + p.m*fluxes)*s2*betaM*DD*(U-eta2);
        ...
        Jac = [-PHI*U       p.sigma*TS      Z              Z;
                thetaM     -p.muB*U         thetaM         thetaM;
                Z           p.sigma*TV1    -PHI*U          Z;
                Z           p.sigma*TV2     Z             -PHI*U];
        
        HR = 0.5*(Jac+Jac');
        
        % TRANSMISSION MATRIX
        TR = [Z     p.sigma*TS*H  Z Z;
              Z     Z           Z Z;
              Z     p.sigma*TV1*H Z Z;
              Z     p.sigma*TV2*H Z Z];
