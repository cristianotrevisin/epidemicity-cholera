...
[cases_AD1_week0, time, y, p, cati_sum, ocv, rain] = M3CMN(x1,x2);
[Rt0, et0] = diagnosis(y, p);

I = y(:,2:11:end) + y(:,7:11:end) + y(:,10:11:end);

for k = 1:size(y,1)
    i0(k) = sqrt(sum((I(k,:)/POPnodes').^2,2));
end
...
ribbon1norm = zeros(nopost,length(i0));
ribbon1et = zeros(nopost,length(i0));
ribbon1Rt = zeros(nopost,length(i0));
opt = "rand";
for j = 1:nopost
    j
    [x1,x2] = get50(opt,rSeq1, rSeq2);  
    ...
    try
        [cases_AD1_week, time, y, p] = M3CMN(x1,x2);
        for k = 1:size(y,1)
                i(k) = sqrt(sum((I(k,:)/POPnodes').^2,2));
        end
        i = i/i(1);
    catch
    ...
    [x1,x2] = get95(opt,rSeq1, rSeq2);  
try
        [~, time, y, p] = M3CMN(x1,x2);
        [Rt, et] = diagnosis(y, p);
        I = y(:,2:11:end) + y(:,7:11:end) + y(:,10:11:end);
    ...
    rib_min_i = zeros(1,length(i0));
    rib_max_i = zeros(1,length(i0));
    rib_min_e = zeros(1,length(i0));
    rib_max_e = zeros(1,length(i0));
    rib_min_r = zeros(1,length(i0));
    rib_max_r = zeros(1,length(i0));
    for i = 1:350
        rib_min_c(i) = min(ribbon1cases(:,i));
        rib_max_c(i) = max(ribbon1cases(:,i));
    end
    for i = 1:length(i0)
        rib_min_i(i) = min(ribbon1norm(:,i));
        rib_max_i(i) = max(ribbon1norm(:,i));
...
f = figure(4002);
    ...
    jbfill(time,rib_max_c,rib_min_c,[0.410 0.41 0.41],'white','FaceAlpha',0.35);
        hold on
